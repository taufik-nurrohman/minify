...
require __DIR__ . D . '..' . D . 'j-s-o-n.php';

$files = glob(__DIR__ . D . 'j-s-o-n' . D . '*.json', GLOB_NOSORT);
usort($files, static function ($a, $b) {
    $a = dirname($a) . D . basename($a, '.json');
    $b = dirname($b) . D . basename($b, '.json');
...
foreach ($files as $v) {
    ...
    $out .= '<h1 id="' . ($n = basename($v, '.json')) . '"><a aria-hidden="true" href="#' . $n . '">&sect;</a> ' . strtr($v, [PATH . D => '.' . D]) . '</h1>';
